...
SAVE_DIR = 'Stick Flag Saved Image'
...
if __name__ == '__main__':
    ...
    parser.add_argument('--src', help="face is sticked", default=IMAGE_PATH, type=str)
    ...
    src = find_file(args.src)
    flag = find_file(args.flag)
    sav = find_file(args.sav)    
    
    face_detections = get_face_detection(src)
    #debug
    ...
    for num, face_detection in enumerate(face_detections):
        ...
        save_image = sav + '/' + get_name_image(args.src) + '/' + 'image_' + str(num)
        make_dir(save_image)
        get_face_mesh(src, x, y, w, h, flag, save_image)